...
class C_IR:
        ...
        for ir in branch:
            if ir.op is MuOp.ARG:
                ...
                if ir.deps[1].op in red_set:
                    ...
                    main.append(("\t" * indent_level) + f"{ir.value} = {self._render_reduce(ir.deps[1])};\n")
                elif ir.deps[1].op is MuOp.TEMP:
                    main.append(("\t" * indent_level) + f"{self._render_load(ir.deps[0])} = {ir.deps[1].value};\n")
                elif ir.deps[1].op in self._bin_set:
                    if ir.deps[0].op is MuOp.T_ARG:
                        main.append(("\t" * indent_level) + f"{ir.deps[0].dtype} {ir.deps[0].value} = {self._render_binary(ir.deps[1])};\n")
                    else:
                        main.append(("\t" * indent_level) + f"{self._render_load(ir.deps[0])} = {self._render_binary(ir.deps[1])};\n")
                elif ir.deps[1].op in un_set:
                    if "#include <math.h>\n" not in headers:
                        headers.append("#include <math.h>\n")
                    main.append(("\t" * indent_level) + f"{self._render_load(ir.deps[0])} = {self._render_unary(ir.deps[1])};\n")
    ...
    def _render_load(self, ir: IR) -> str:
        if ir.op is MuOp.T_ARG or ir.op is MuOp.TEMP:
            return f"{ir.value}"
        elif len(ir.deps) == 1:
...
class CUDA_IR:
    ...
    def to_code(self, branch: list[IR]) -> None:
        headers = ["#include <stdlib.h>\n"]
        macros = []
        main = [f"__global__ void {self.name}("]
        indent_level = 0
        store = ""
        ...
        for ir in branch:
            if ir.op is MuOp.ARG:
                ...
                if ir.deps[1].op is MuOp.FL_DIV:
                    main.append(("\t" * indent_level) + f"int {ir.deps[0].value} = ({ir.deps[1].deps[0].deps[0].value} % {ir.deps[1].deps[0].deps[1].value}) / {ir.deps[1].deps[1].value};\n")
                elif ir.deps[1].op in red_set:
                    if "#include <math.h>\n" not in headers:
                        headers.append("#include <math.h>\n")
                    if ir.deps[1].op is MuOp.MAX:
                        if "#define MAX(x, y) (((x) > (y)) ? (x) : (y))\n" not in macros:
